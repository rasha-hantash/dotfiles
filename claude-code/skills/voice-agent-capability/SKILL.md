...
4. **Structural over prompt-level:** filter data server-side, not in prompts; a tool that must never fire gets detached, not warned against (gpt-4.1 fires attached tools regardless). For per-org EMR field hiding/exposure (which layer, key shape, config-surface, fail-safe default), use the `emr-data-exposure` skill.
