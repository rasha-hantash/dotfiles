...
6. **New/renamed agents need guardrails synced to Hamming** — an agent's `monitoring.yaml` guardrails + custom metrics only take effect in Hamming after `agents-tests-runner/sync_monitoring.sh --env <env> --org-id <uuid> --agent <dir>`. This is a **separate step** from VAPI deploy and from test runs; an agent can be fully live and still have zero guardrails. Symptom of a missed sync: `No Hamming agent found with name '<PREFIX> - <AGENT>'` or absent guardrails in monitoring.
